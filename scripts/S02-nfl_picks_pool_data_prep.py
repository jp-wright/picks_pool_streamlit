...
from py_modules.io_utilities import export_to_csv
...
class DataPrepper():
    ...
    def __init__(self):
        ...
        # self.LOCAL_PATH = os.path.join(ROOT_PATH, "local_projects")
        self.REPO_LOCAL = os.environ['NFLPICKS_LOCAL_REPO']
        self.LOCAL_PATH = os.environ['NFLPICKS_LOCAL_PROJ']
    ...
    def prep_year_data_for_website(self, dfy: pd.DataFrame) -> pd.DataFrame:
        ...
        export_to_csv(frame, fname='year_data_current.csv', path=self.LOCAL_PATH, subdir='data/output/email_tables', ovrwrt=True)
        export_to_csv(frame, fname='year_data_current.csv', path=self.REPO_LOCAL, subdir='data/output/email_tables', ovrwrt=True)
        return frame
    ...
    def prep_round_data_for_website(self, dfr: pd.DataFrame):
        ...
        frame.columns = [c.replace('_', ' ') for c in frame.columns]
        export_to_csv(frame, fname='round_data.csv', path=self.LOCAL_PATH, subdir='data/output/email_tables', ovrwrt=True)
        export_to_csv(frame, fname='round_data.csv', path=self.REPO_LOCAL, subdir='data/output/email_tables', ovrwrt=True)
        return frame
    ...
    def prep_career_data_for_website(self, dfc: pd.DataFrame):
        ...
        frame.columns = [c.replace('_', ' ') for c in frame.columns]
        export_to_csv(frame, fname='career_data.csv', path=self.LOCAL_PATH, subdir='data/output/email_tables', ovrwrt=True)
        export_to_csv(frame, fname='career_data.csv', path=self.REPO_LOCAL, subdir='data/output/email_tables', ovrwrt=True)
        return frame
    ...
    def prep_year_history(self, dfy: pd.DataFrame, highlight_year: int):
        ...
        export_to_csv(pd.concat(frames, axis=1), fname='reg_po_tot_top10.csv', path=self.LOCAL_PATH, subdir='data/output/email_tables', ovrwrt=True)
        export_to_csv(pd.concat(frames, axis=1), fname='reg_po_tot_top10.csv', path=self.REPO_LOCAL, subdir='data/output/email_tables', ovrwrt=True)
        return frames
    ...
    def prep_player_history(self, dfy: pd.DataFrame, highlight_year: int):
        ...
        export_to_csv(frame, fname='year_data_history.csv', path=self.LOCAL_PATH, subdir='data/output/email_tables', ovrwrt=True)
        export_to_csv(frame, fname='year_data_history.csv', path=self.REPO_LOCAL, subdir='data/output/email_tables', ovrwrt=True)
        return frame
    ...
    def prep_manager_teams_this_year(self, df: pd.DataFrame, curr_year: int):
        ...
        export_to_csv(frame, fname='manager_teams_data.csv', path=self.LOCAL_PATH, subdir='data/output/email_tables', ovrwrt=True)
        export_to_csv(frame, fname='manager_teams_data.csv', path=self.REPO_LOCAL, subdir='data/output/email_tables', ovrwrt=True)
        return frame
    ...
    def prep_champ_history(self, dfy: pd.DataFrame, highlight_year: int):
        ...
        frame[int_cols] = frame[int_cols].astype(int)
        export_to_csv(frame, fname='champ_data.csv', path=self.LOCAL_PATH, subdir='data/output/email_tables', ovrwrt=True)
        export_to_csv(frame, fname='champ_data.csv', path=self.REPO_LOCAL, subdir='data/output/email_tables', ovrwrt=True)
        return frame
    ...
    def prep_playoff_teams_this_year(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        export_to_csv(frame, fname='playoff_teams_data.csv', path=self.LOCAL_PATH, subdir='data/output/email_tables', ovrwrt=True)
        export_to_csv(frame, fname='playoff_teams_data.csv', path=self.REPO_LOCAL, subdir='data/output/email_tables', ovrwrt=True)
        return frame
    ...
    def prep_best_worst_picks_by_rd(self, df):
        ...
        export_to_csv(frame, fname='best_worst_picks_data.csv', path=self.LOCAL_PATH, subdir='data/output/email_tables', ovrwrt=True)
        export_to_csv(frame, fname='best_worst_picks_data.csv', path=self.REPO_LOCAL, subdir='data/output/email_tables', ovrwrt=True)
        return frame
