...
class PageLayoutSeason(DataPrepper):
    """Layout class 
    """
    ...
    def manager_wins_by_round(self):
        ...
        styled_df = stylr.create_styled_frame(self.dfpt.copy(), self.names_regex, bg_clr_dct)
        ...
        with st.container():
            ...
            with col2:
                ...
                # st.dataframe(stylr.style_frame(frame, bg_clr_dct=bg_clr_dct, kind='streamlit'), hide_index=True, use_container_width=True)
                # st.table(stylr.style_frame(frame, bg_clr_dct=bg_clr_dct, kind='streamlit'))
    ...
    def manager_projwins_by_round(self):
        ...
        styled_df = stylr.create_styled_frame(self.dfproj.copy(), self.names_regex, bg_clr_dct)
        with st.container():
            _, col2, _ = st.columns([.05, .9, .05])
            with col2:
                st.write(styled_df.to_html(), unsafe_allow_html=True, hide_index=True)
